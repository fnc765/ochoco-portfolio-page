...
test('E-U4: 撮影でラベルが「再撮影」になり、resultCanvas に描画される', async ({ page }) => {
    ...
    // 撮影後は result-canvas が photo-frame を覆い、二重枠を作らないこと
    const layout = await page.evaluate(() => {
        ...
        return {
            ...
            resultW: rr.width, resultH: rr.height,
            resultTop: rr.top - fr.top,
            resultLeft: rr.left - fr.left,
            contentVisibility: cs.visibility,
        };
    });
    // result-canvas が photo-frame をほぼ覆っている（数pxの違いは許容）
    expect(Math.abs(layout.resultW - layout.frameW)).toBeLessThanOrEqual(2);
    expect(Math.abs(layout.resultH - layout.frameH)).toBeLessThanOrEqual(2);
    expect(Math.abs(layout.resultTop)).toBeLessThanOrEqual(2);
    expect(Math.abs(layout.resultLeft)).toBeLessThanOrEqual(2);
    // frame-content は hidden（黒背景を消して二重枠を防ぐ）
    expect(layout.contentVisibility).toBe('hidden');
});
