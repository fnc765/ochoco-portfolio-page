...
1ページUI（3画面遷移は廃止）。シャッターボタンの状態機械でカメラ起動・撮影・再撮影を制御する。
...
LIVE ─(撮影)─▶ 撮影処理
  ...
  ↓ resultCanvas に焼き込み、video/overlay 非表示
CAPTURED（ラベル「再撮影」、共有/保存ボタン enabled）
  ↓
CAPTURED ─(再撮影)─▶ STARTING ▶ LIVE

[並行操作（撮影前後で常に有効）]
  ・画像選択（ファイル or 履歴モーダル）: カメラ起動と独立
  ・入力画像調整スライダー（折りたたみ）: 撮影前=overlay-canvas、撮影後=resultCanvas
  ・メタ情報入力（タイトル/撮影者/日付/場所）: 撮影後ならresultCanvasにも即時反映
  ・共有/PNG保存ボタン: 撮影後のみ有効、場所入力時ワーニング
```
