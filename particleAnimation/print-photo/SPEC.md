...
| 項目 | 仕様 |
|---|---|
| **入力方法** | `<input type="file" accept="image/png,image/jpeg,image/webp">` でファイル選択 |
| **対応形式** | PNG / JPEG / WebP（1ファイル25MB以下）。透過を利用する場合はアルファチャンネル付きPNGまたはWebPを推奨。作業用Canvasは最大辺4096pxに縮小 |
...
| **サムネイルキャッシュ** | `IndexedDB` で過去に読み込んだ画像を最大辺1024pxの透過PNGへ縮小して保存。新しい順に上限**10件**。手動削除可能 |
...
| **画像調整** | 入力画像に対して明るさ/コントラスト/色温度スライダーを配置。色温度は-100〜+100、+で暖色、-で寒色（Canvas filter方式） |
| **合成エリア** | 16:9で固定、フレーム内上寄り配置 |

### 4.3 テキスト入力（常時表示）

1ページUIのため、撮影前後に関わらずメタ情報入力タイル（タイトル・撮影者・日付・場所）は常に表示される。撮影後のテキスト編集は `resultCanvas` にも即時反映される。
...
| **出力フォーマット** | PNG（透過入力と画質を維持） |
| **保存** | `a[download]` 属性でローカルダウンロード |
| **共有** | ① **Web Share API** (`navigator.share({ files: [blob] })`) — 対応端末ではOS共有シート経由でX等へ画像付き共有<br>② **X Intent** (`https://x.com/intent/post`) — Web Share非対応時にテキスト＋ハッシュタグを事前入力し、画像は手動添付 |
...
1ページUI（3画面遷移は廃止）。シャッターボタンの状態機械でカメラ起動・撮影・再撮影を制御する。**撮影前後の表示は HTML（白額縁・黒絵エリア・テキスト）を表示器として継続使用し、resultCanvas のような合成済み画像は画面に出さない。** 撮影時の最終合成（2048x1440、白額縁+絵+テキスト）は内部キャンバスとしてメモリに保持し、保存/共有でのみ利用する。

```
[初期状態]
  ↓ 画像未選択でもシャッターボタンは「カメラを起動」(enabled)
  ↓ プレビュー枠にプレースホルダ（カメラアイコン+「画像を選択してください」）

[シャッターボタン click]
  ↓
STARTING ─ok─▶ LIVE（カメラ映像表示、合成可能）
  err ─▶ ERROR（ガイド表示）
LIVE ─(撮影)─▶ 撮影処理
  ↓ 成功 → stopCameraInternal(true) で完全停止
  ↓ 内部キャンバス (2048x1440) を renderFrame で生成・保持
  ↓ 画面表示は撮影前と同じ HTML 構造がそのまま残る
CAPTURED（ラベル「再撮影」、共有/保存ボタン enabled）
  ↓
CAPTURED ─(再撮影)─▶ 内部キャンバス破棄 → STARTING ▶ LIVE

[並行操作（撮影前後で常に有効）]
  ・画像選択（ファイル or 履歴モーダル）: カメラ起動と独立
  ・入力画像調整スライダー（折りたたみ）: 撮影前=overlay-canvas のみ。
    撮影後=内部キャンバスを「縮小版 (512x361) で即時生成 + 最終 2048x1440 を rAF でスケジュール」
    change イベントで最終版を即時確定
  ・メタ情報入力（タイトル/撮影者/日付/場所）: 撮影後なら内部キャンバスを再生成
  ・共有/PNG保存ボタン: 撮影後のみ有効、内部キャンバスを PNG/Blob で出力。場所入力時ワーニング
```
...
1ページUI構成への再編を反映した実装順序:

1. **フェーズ1**: 1ページUI骨組み（HTML/CSS、stickyプレビュー、details折りたたみ、ツールバー）
2. **フェーズ2**: シャッターボタン状態機械（IDLE/STARTING/LIVE/CAPTURED/ERROR）
3. **フェーズ3**: 画像選択とカメラ起動の独立（applyImage 共通化、履歴モーダル）
4. **フェーズ4**: 撮影・再撮影フロー（resultCanvas 表示切替、stopCameraInternal(true)）
5. **フェーズ5**: 入力画像調整の常時有効化（撮影後スライダー反映、renderResultFromState）
6. **フェーズ6**: メタ情報入力と撮影後テキスト反映（resultCanvas 再描画）
7. **フェーズ7**: 保存/共有（2ボタン構成、ワーニングモーダル、Web Share → X Intent）
8. **フェーズ8**: テスト全面更新（Vitest維持、Playwright新ヘルパー・新ID体系）
